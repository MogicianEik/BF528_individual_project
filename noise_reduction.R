...
required_var <- qchisq(0.01,df= 133, ncp=mean_var ) # find the 99th percentile of the chi-squared distribution with n degrees of freedom
test <- test[which(rowVars(test) < required_var ),]
# filter by row, for each row, the coefficient of variation is greater than 0.186
test <- test[which(apply(test, 1, function(x) sd(x) / mean(x) * 100) > 0.186),]
write.csv(test,'write.csv')
